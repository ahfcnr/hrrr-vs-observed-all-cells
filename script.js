...
function updatePlot() {
    ...
    // 2. Prepare Forecast Traces
    const cells = [...new Set(filtered.map(d => d.cell_id))];
    let forecastTraces = [];
    cells.forEach(cell => {
        const cellData = filtered
            .filter(d => d.cell_id === cell)
            .sort((a, b) => new Date(a.valid_time) - new Date(b.valid_time));

        forecastTraces.push({
            x: cellData.map(d => d.valid_time),
            y: cellData.map(d => d.rain),
            mode: "lines",
            name: cell
        });
    });

    // 3. Add Observed Reference Line to Forecast Plot (Dotted Line)
    forecastTraces.push({
        x: obsTimes,
        y: obsValues,
        mode: "lines",
...
// Final Y max = ceil(max of both)
    ...
    // 5. Render Forecast Plot (includes dotted reference line)
    Plotly.newPlot("forecastPlot", forecastTraces, {
        ...sharedLayout,
        title: `HRRR Forecast – ${region}`
    });

    // 6. Render Separate Observed Plot (Original Solid Line)
    const observedTrace = {
        x: obsTimes,
        y: obsValues,
        mode: "lines",
